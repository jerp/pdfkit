...
class PDFFont {
  static registerFontFormat(fontFormatClass) {
    formatFormats[fontFormatClass.name] = fontFormatClass
  }
  static open(document, src, family, id) {
    const StandardFont = formatFormats.StandardFont
    if (typeof src === 'string' && StandardFont.isStandardFont(src)) {
      return new StandardFont(document, src, id)
    } else {
      const EmbeddedFont = formatFormats.EmbeddedFont
