...
module.exports = {
  ...
  _initOptions(x, y, options = {}) {
    ...
    if (this._textOptions) {
      Object.keys(this._textOptions).forEach(key => {
        if (key !== 'continued' && options[key] == null) {
          options[key] = this._textOptions[key]
        }
      })
    }
  ...
  _line(text, options, wrapper) {
    ...
    if (!wrapper) {
      this.x += this.widthOfString(text)
    } else {
      this.y += this.currentLineHeight(true) + lineGap
    }
  },
  ...
  _fragment(text, x, y, options) {
    ...
    if (wordSpacing) {
      ...
      words.forEach(word => {
        ...
        const space = Object.assign({}, positions[positions.length - 1])
